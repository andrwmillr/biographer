...
def _build_preface_kickoff(run_dir_abs: Path, user_msg: str, corpus_id: str) -> str:
    ...
    return (
        ...
        f"Output file (Phase 2 only): {run_dir_abs}/output.md\n\n"
        "Follow your system prompt's two-phase process. Phase 1: read "
        "everything, then propose a structure (opening image, spine, key "
        "quotes, closing move) in your chat response. Do NOT write to "
        "output.md yet. Wait for the user to approve or redirect.\n\n"
        "Don't list directories, read sibling files, or browse anywhere "
        "else. The inlined block is your full context.\n\n"
        "Start reading immediately. No preamble questions. The user is "
        "watching live — begin narrating what you notice as you read "
        "through the chapters. Name specific moments, threads, and "
        "connections. Then deliver the proposal.\n\n"
        "--- INPUT-START ---\n\n"
        + user_msg
        + "\n\n--- INPUT-END ---\n"
    )

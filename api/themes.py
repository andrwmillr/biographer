...
def _build_themes_kickoff(run_dir_abs: Path, corpus_sample: str, corpus_id: str | None) -> str:
    ...
    return (
        ...
        "using the Write tool in the same format as the LOCKING section of your "
        "system prompt (### name, gloss, eras, scoped notes). "
        "This populates the Draft pane for the user. "
        "Do NOT output '[locked]' — this is just the initial draft, not a "
        "finalization.\n\n"
        "**Then transition to curate mode:** end with the single line "
        '"Ready for your moves." Wait for the user.\n\n'
        f"Only output '[locked] wrote themes.md.' when you receive `/lock` later. "
        f"On `/lock`, overwrite {run_dir_abs}/themes.md with the "

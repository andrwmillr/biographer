#!/usr/bin/env python3
"""Scan creative-labeled notes (plus any notes in Poetry-named paths) and
classify each as: poem, contains_poem (embedded in other prose), or not_poem.

Output: _corpus/_poems.tsv
"""
import asyncio
import json
import os
...
OUT_TSV = CORPUS / "_derived" / "_poems.tsv"
...
async def main():
    ...
    # Candidates: creative-labeled notes UNION notes in Poetry-ish paths
    candidates = set()
    ...
    for coro in asyncio.as_completed(tasks):
        ...
        print(f"[{done_count}/{len(queue)}] {marker} {r['classification']:<14} {r['rel'][:90]}")
